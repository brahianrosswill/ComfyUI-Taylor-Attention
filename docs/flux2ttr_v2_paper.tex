...
Pure kernel regression attention, while theoretically sound and linear-time, has known failure modes inherited from all linear attention variants: it struggles with sharp retrieval (where one key should dominate), rare correlations, and compositional binding~\citep{arora2024}. These are precisely the interactions where the softmax's exponential amplification matters most. Rather than accepting this quality gap, v2 compensates with a small exact-attention residual computed against a sparse set of landmark tokens. The full architecture is:
\begin{equation}\label{eq:hybrid}
  \boxed{\;\text{Output}
  = \underbrace{\text{Normalized Kernel Linear Attention}}_{\text{Branch A (linear-time)}}
  + \;\alpha\cdot\underbrace{\text{Landmark Softmax Residual}}_{\text{Branch B (quality rescue)}}\;}
\end{equation}

\subsubsection{Branch A: Nadaraya--Watson Kernel Attention}

Branch~A computes normalized kernel regression in linear time using chunked accumulation. The learned positive feature map $\bphi$ is implemented as an MLP followed by an $\ELU{+}1$ activation:
\begin{equation}\label{eq:phi}
  \bphi(\mathbf{x}) = \ELU\bigl(\MLP(\mathbf{x})\bigr) + 1\,,
\end{equation}
which guarantees strictly positive features. Separate feature networks $\bphi_q$ and $\bphi_k$ are optionally supported but we default to shared weights. The computation proceeds in two phases:
...
Branch~B performs exact softmax attention from all queries to a small set of $M$ landmark tokens (typically 64--256). Landmark selection is deterministic: all text conditioning tokens are included (as they carry critical semantic information), plus uniformly-spaced image tokens to fill the remaining budget. Effective landmarks scale dynamically:
\begin{equation}\label{eq:landmarks}
  M_\text{eff} = \mathrm{clamp}\bigl(\lfloor n\cdot f\rceil,\; M_\text{min},\; M_\text{max}\bigr)
\end{equation}
where $f$ is a configurable fraction (default~0.08), $M_\text{min}=64$, $M_\text{max}=512$.

The landmark attention cost is $O(n\cdot M\cdot d)$, which for $M=128$ and $n=16{,}384$ ($1024\times1024$ image) is approximately $128\times$ cheaper than full $n\times n$ attention. The landmark output is mixed in with a per-head learnable scalar~$\alpha$, initialized near zero so that the kernel branch dominates during early training:
\begin{equation}\label{eq:gate}
  \by_i = \by_i^{\,\text{kernel}} + \alpha\cdot\by_i^{\,\text{landmark}}\,.
\end{equation}

The landmark branch serves as a ``quality rescue'' mechanism, compensating for the known limitations of kernel attention by providing a small dose of exact softmax computation at the tokens most likely to carry globally-relevant information. Because the landmark set always includes text tokens, the system preserves the text-to-image cross-attention pathways that are critical for prompt adherence.
...
Training on consecutive attention calls within a single denoising trajectory produces highly correlated gradients. We address this with per-layer replay buffers that store $(\bq_\text{sub}, \bK, \bV, \by_\text{teacher})$ tuples from previous steps, globally budgeted across all layers to a configurable memory cap (default 768\,MB) with oldest-sample eviction. Multiple training steps draw random minibatches from the buffer, providing temporal diversity across denoising timesteps and reducing overfitting to any single noise level.

Each layer maintains an exponential moving average of its training loss:
\begin{equation}
  \bar{\mathcal{L}}_t = \beta\,\bar{\mathcal{L}}_{t-1} + (1-\beta)\,\mathcal{L}_t\,,
  \qquad \beta=0.9\,.
\end{equation}
A layer is flagged as ``ready'' for deployment only when $\bar{\mathcal{L}} < \tau$ (configurable threshold) and it has accumulated a minimum number of training updates. Non-ready layers are clamped to full attention during inference regardless of the controller's output.

\subsection{Loss Function and Optimization}

The training loss combines three terms:
\begin{equation}\label{eq:loss}
  \mathcal{L} = w_\text{rmse}\cdot\mathcal{L}_\text{RMSE}
              + w_\text{cos}\cdot(1 - \cos\text{-sim})
              + w_\text{lpips}\cdot\mathcal{L}_\text{LPIPS}
\end{equation}
where RMSE captures overall magnitude, cosine dissimilarity captures directional alignment in the high-dimensional output space, and LPIPS~\citep{zhang2018} (computed by decoding both outputs to pixel space) ensures the distillation preserves perceptually-relevant structure. Optimization uses AdamW with separate learning rates for the feature-map parameters (lower) and the gate/$\alpha$ parameters (higher), plus gradient clipping at norm~1.0 for stability.
...
Once TTR layers have been distilled, a binary controller is trained to decide per-layer, per-denoising-step routing. The controller runs dual-path sampling: the original model as teacher versus the TTR-patched model as student. It computes quality deltas ($\Delta$RMSE, $\Delta$cosine, $\Delta$LPIPS) between the two paths and updates its routing policy via REINFORCE~\citep{williams1992}, with the reward balancing image quality against a configurable efficiency target (the fraction of layers routed to TTR).

The controller uses Gumbel-Softmax relaxation during training, annealing temperature from a warm start to near-deterministic selection. At inference, a quality/speed slider maps to a probability threshold: $0.0$ favors maximum TTR usage (threshold~$0.1$), $1.0$ favors full attention (threshold~$0.9$). Efficiency targets are computed only over readiness-qualified layers, with both eligible-layer and overall ratios logged for interpretability.

Critically, the controller currently operates only on layer index and diffusion timestep ($\sigma$) information. It does not examine text tokens or image content. We believe this is a significant limitation: intuitively, some prompts require more precise attention (e.g., detailed spatial compositions, text rendering) while others are more tolerant of approximation (e.g., simple scenes, abstract imagery). Future work will extend the controller with a small MLP that extracts signal from the text conditioning tokens to guide routing decisions.
...
After a few hours of online distillation during normal image generation workflows, approximately half of Flux2's single-stream attention layers converge to EMA losses below the deployment threshold. These layers span primarily the middle and later blocks, consistent with the intuition that early layers (which establish coarse global structure) require more precise attention, while later layers (which refine local detail within an already-structured representation) tolerate the kernel regression approximation. The aggregate training snapshots logged every 10~updates show clear loss convergence with q25--q75 ranges for per-layer loss, cosine similarity, and NMSE all tightening over time.

\subsection{Image Quality}

Images generated with TTR~v2 substitution on ready layers are in many cases coherent enough to be used. The most common degradation is a subtle softening of fine texture and high-frequency detail, consistent with the kernel attention's smoothing bias. Prompt adherence, global composition, and color fidelity are largely preserved, likely thanks to the landmark branch's inclusion of all text tokens in its exact-attention set.

\subsection{Controller Routing}

The REINFORCE-trained controller learns meaningful routing patterns: it routes more layers to TTR during early denoising steps (high~$\sigma$ / high noise, where attention patterns are diffuse) and falls back to full attention during later steps (low~$\sigma$, where fine detail matters). This sigma-dependent routing emerges naturally from the quality/efficiency reward signal without explicit scheduling.
...
Learned MLP + $\ELU{+}1$ (guaranteed positive) \\[4pt]
Feature dimension &
...
Landmark softmax residual (64--512 tokens, learnable gate) \\[4pt]
Distillation &
Joint Q/K/V subsampling (internally inconsistent) &
Query-only subsampling; full K/V preserved \\[4pt]
Practical result &
